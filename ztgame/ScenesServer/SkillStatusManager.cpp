		...
		case ACTION_STEP_RELOAD:
			{
				if(zMisc::selectByPercent((int)(sse.percent)))
				{
					uint64_t value = (uint64_t)(pEntry->getMaxMP()*(sse.value/100.0f)); //by=>friday 修复MP计算，使用64位无符号类型防止溢出
		...
		case ACTION_STEP_TIMER:
			{
				...
				if (pAtt)
				{
					uint64_t value = (uint64_t)(pEntry->getMaxHP()*(sse.value/100.0f)); //by=>friday 修复技能伤害计算，使用64位无符号类型防止溢出
		...
		case ACTION_STEP_RELOAD:
			{
				if(zMisc::selectByPercent((int)(sse.percent)))
				{
					pEntry->changeHP((uint64_t)(pEntry->getMaxHP()*(value/100.0f))); //by=>friday 修复技能回血计算，使用64位无符号类型防止溢出
					return SKILL_ACTIVE;
				}
				else
					return SKILL_RETURN;
		...
		case ACTION_STEP_RELOAD:
			{
				if(zMisc::selectByPercent((int)(sse.percent)))
				{
					...
					pEntry->skillValue.appenddam += (uint64_t)(pAtt->getMaxHP()*(sse.value/100.0f)); //by=>friday 修复技能伤害计算，使用64位无符号类型防止溢出
					return SKILL_ACTIVE;
				}
				else
					return SKILL_RETURN;
		...
		case ACTION_STEP_START:
			{
				if (pEntry->summon && ((pEntry->summon->id >= (DWORD)sse.percent)&&(pEntry->summon->id < (DWORD)sse.percent+10)))
				{
					pEntry->skillValue.introject_maxmdam = (uint64_t)(pEntry->summon->getMaxMDamage()*sse.value/100.0f); // 召唤合体增加魔法攻击
					pEntry->skillValue.introject_maxpdam = (uint64_t)(pEntry->summon->getMaxPDamage()*sse.value/100.0f); // 召唤合体增加物理攻击
					pEntry->skillValue.introject_mdam	 = (uint64_t)(pEntry->summon->getMinMDamage()*sse.value/100.0f); // 召唤合体增加魔法攻击
					pEntry->skillValue.introject_pdam	 = (uint64_t)(pEntry->summon->getMinPDamage()*sse.value/100.0f); // 召唤合体增加物理攻击
					pEntry->skillValue.introject_mdef	 = (uint64_t)((pEntry->summon->getMaxMDefence()*sse.value/100.0f)*0.5f); // 召唤合体增加魔法防御
					pEntry->skillValue.introject_pdef	 = (uint64_t)((pEntry->summon->getMaxPDefence()*sse.value/100.0f)*0.5f); // 召唤合体增加物理防御
					pEntry->skillValue.introject_maxhp	 = (uint64_t)((pEntry->summon->getMaxHP()*sse.value/100.0f)); // 召唤合体增加物理防御 //by=>friday 修复技能伤害计算，使用64位无符号类型防止溢出
					pEntry->killSummon();
					if (pEntry->getType() == zSceneEntry::SceneEntry_Player)
					{
						SceneUser *pUser = (SceneUser *)pEntry;
		...
		case ACTION_STEP_RELOAD:
			{
				if(zMisc::selectByPercent((int)(sse.percent)))
				{
					pEntry->skillValue.pdeftodam = (uint64_t)(SKPDefence *(cevalue/100.0f)); //by=>friday 修复防御力转攻击力计算，使用64位无符号类型防止溢出
		...
		case ACTION_STEP_RELOAD:
			{
	            if(zMisc::selectByPercent((int)(sse.percent)))
				{
					pEntry->skillValue.mdeftodam = (uint64_t)(SKMDefence *(cevalue/100.0f)); //by=>friday 修复防御力转攻击力计算，使用64位无符号类型防止溢出
...
BYTE SkillStatus_263(SceneEntryPk *pEntry, SkillStatusElement &sse)
{
	...
	switch(sse.byStep)
	{
		...
		case ACTION_STEP_RELOAD:
			{
				pEntry->skillValue.protectdam = (uint64_t)(pEntry->getMaxHP()*(sse.percent/100.0f)); //by=>friday 修复技能伤害计算，使用64位无符号类型防止溢出
