 ...
 */
 ...
 bool Upgrade::purple_upgrade(SceneUser& user, zObject* ob, int extra_odds)
 {
	   ...
	   if (ob->data.upgrade < 35 && (ob->data.kind & 16 || !ob->data.kind & 4) && ob->base->kind != ItemType_Amulets)
	   {
 ...
 */
 ...
 /**   
  * \brief 定情信物镶嵌升级物品 //by=>friday
  *
  * \param user:定情信物镶嵌的用户
  * \param ob: 定情信物物品
  * \param extra_odds: 额外成功率
  * \return 升级成功返回true,失败返回false
  */
 bool Upgrade::dqxw_upgrade(SceneUser& user, zObject* ob, int extra_odds)
 {
		 
	//  zUdqxwxqObjectB *base = udqxwxqobjectbm.get(ob->data.dwObjectID + 100000 * (ob->data.retain100 + 1));
	 zUdqxwxqObjectB *base = udqxwxqobjectbm.get(ob->data.dwObjectID );
	 if(!base) return false;
	 
	 if (zMisc::selectByPercent(base->odds + extra_odds)) 
	 {
		 do_dqxw_upgrade(ob, base);
		 return true;
	 }
	 else 
	 {
		 ...
		 for( int i=0;i<down;i++)
		 {
			 base = udqxwxqobjectbm.get(ob->data.dwObjectID);
			 if (base) do_dqxw_downgrade(ob, base);
		 }
		 return false;
	 }
 }
  ...
  * \return 升级成功返回true,失败返回false
  */
 bool Upgrade::xyts_upgrade(SceneUser& user, zObject* ob, int extra_odds)
 {
		 
	 zUxytsObjectB *base = uxytsobjectbm.get(ob->data.dwObjectID + 100000 * (ob->data.retain101 + 1));
	 if(!base) {
		 return false;
	 }
	//  Zebra::logger->debug("心意提升成功机率为%d", base->odds + extra_odds);
	 if (zMisc::selectByPercent(base->odds + extra_odds)) 
	 {
		 do_xyts_upgrade(ob, base);
		 
		 //by=>friday, 升心成功时更新历史最高心
		 if (ob->data.retain101 > ob->data.retain104)
		 {
			 ob->data.retain104 = ob->data.retain101;
		 }
	 ...
	 {
		 //by=>friday, 升心失败时处理心心掉落（20%几率）
		 int random = zMisc::randBetween(1, 100); //by=>friday, 生成1-100的随机数
		 if (random <= 30) //by=>friday, 30%几率掉落心心
		 {
			 DWORD currentHearts = ob->data.retain101;
			 DWORD dropAmount = 1; //by=>friday, 默认掉落1心
			 
			 //by=>friday, 根据当前心心数量确定掉落数量
			 if (currentHearts <= 50)
			 {
				 dropAmount = 1; //by=>friday, <=50心失败掉落1心
			 }
			 else if (currentHearts >= 51 && currentHearts <= 80)
			 {
				 dropAmount = 2; //by=>friday, 51-80心失败掉落2心
			 }
			 else if (currentHearts > 80)
			 {
				 dropAmount = 3; //by=>friday, >80心失败掉落3心
			 }
			 
			 for (DWORD i = 0; i < dropAmount && ob->data.retain101 > 0; i++)
			 {
				 base = uxytsobjectbm.get(ob->data.dwObjectID + 100000 * ob->data.retain101);
				 if (base) do_xyts_downgrade(ob, base);
			 }
		 }
		 
		 //by=>friday, 历史最高心不变，保持retain104的值
		 return false;
	 }
 }

 /**   
  * \brief 情谊提升升级物品 //by=>friday
  *
  * \param user:情谊提升的用户
  * \param ob: 装备物品
  * \param extra_odds: 额外成功率
  * \return 升级成功返回true,失败返回false
  */
 bool Upgrade::qyts_upgrade(SceneUser& user, zObject* ob, int extra_odds)
 {
		 
	 zUqytsObjectB *base = uqytsobjectbm.get(ob->data.dwObjectID + 100000 * (ob->data.retain102 + 1));
	 if(!base) return false;
	 
	 if (zMisc::selectByPercent(base->odds + extra_odds)) 
	 {
		 do_qyts_upgrade(ob, base);
		 return true;
	 }
	 else 
	 {
		 ...
		 for( int i=0;i<down;i++)
		 {
			 base = uqytsobjectbm.get(ob->data.dwObjectID + 100000 * ob->data.retain102);
			 if (base) do_qyts_downgrade(ob, base);
		 }
		 return false;
	 }
 }
 ...
 /**   
  * \brief 定情信物进阶升级物品 //by=>friday
  *
  * \param user:定情信物进阶的用户
  * \param ob: 装备物品
  * \param extra_odds: 额外成功率
  * \return 升级成功返回true,失败返回false
  */
 bool Upgrade::dqxwjj_upgrade(SceneUser& user, zObject* ob, int extra_odds)
 {
		 
	 zUdqxwjjObjectB *base = udqxwjjobjectbm.get(ob->data.dwObjectID + 100000 * (ob->data.retain103 + 1));
	 if(!base) return false;
	 
	 if (zMisc::selectByPercent(base->odds + extra_odds)) 
	 {
		 do_dqxwjj_upgrade(ob, base);
		 return true;
	 }
	 else 
	 {
		 ...
		 for( int i=0;i<down;i++)
		 {
			 base = udqxwjjobjectbm.get(ob->data.dwObjectID + 100000 * ob->data.retain103);
			 if (base) do_dqxwjj_downgrade(ob, base);
		 }
		 return false;
	 }
 }
 ...
 //soke 历史最高星数 //by=>friday 修改为支持无限星数
DWORD targetStar = ob->data.upgrade + 1; //by=>friday 目标星数等于升级等级+1
if (ob->data.xingshu < targetStar) //by=>friday 如果当前历史最高星数小于目标星数
{
    ob->data.xingshu = targetStar; //by=>friday 直接设置为目标星数，支持无限星数
}
	 ++ob->data.upgrade;
	 //soke 升星不改变需要的等级
	 //++ob->data.needlevel;
 ...
 */
 ...
 #define COMPUTE(x) ob->data.x += base->x;
 /**     
  * \brief 定情信物镶嵌升级 //by=>friday
  *
  * \param ob: 装备物品
  * \param base: 定情信物镶嵌属性
  * \return 当前总是返回true
  */
 bool Upgrade::do_dqxw_upgrade(zObject* ob, zUdqxwxqObjectB* base)
 {
	 //增加定情信物镶嵌标记 //by=>friday
	 ++ob->data.retain100;
	 COMPUTE( p26damage )				// 定情信物镶嵌最小攻击力
	 COMPUTE( m26axpdamage )			// 定情信物镶嵌最大攻击力
	 COMPUTE( m26damage )			    // 定情信物镶嵌最小法术攻击力
	 COMPUTE( m26axmdamage )			// 定情信物镶嵌最大法术攻击力

	 COMPUTE( p26defence )			    // 定情信物镶嵌物防
	 COMPUTE( m26defence )			    // 定情信物镶嵌魔防
	 COMPUTE( m26axhp )			        // 定情信物镶嵌最大生命值
	 
	 // 绝技攻防计算 //by=>friday
	 COMPUTE( jj26_attack )
	 COMPUTE( jj26_defence )
 ...
 #undef COMPUTE
 #define COMPUTE(x) ob->data.x -= base->x; \
 if ((SWORD)ob->data.x < 0) ob->data.x = 0;

 /**     
  * \brief 定情信物镶嵌降级 //by=>friday
  *
  * \param ob: 装备物品
  * \param base: 定情信物镶嵌属性
  * \return 当前总是返回true
  */
 bool Upgrade::do_dqxw_downgrade(zObject* ob, zUdqxwxqObjectB* base)
 {
	 if (ob->data.retain100 > 0) 
	 {
		 --ob->data.retain100;	
 ...
 #define COMPUTE(x) ob->data.x += base->x;
 /**     
  * \brief 心意提升升级 //by=>friday
  *
  * \param ob: 装备物品
  * \param base: 心意提升属性
  * \return 当前总是返回true
  */
 bool Upgrade::do_xyts_upgrade(zObject* ob, zUxytsObjectB* base)
 {
	 //增加心意提升标记 //by=>friday
	 ++ob->data.retain101;
	 
	 COMPUTE( p27damage )				// 心意提升最小攻击力
	 COMPUTE( m27axpdamage )			// 心意提升最大攻击力
	 COMPUTE( m27damage )			    // 心意提升最小法术攻击力
	 COMPUTE( m27axmdamage )			// 心意提升最大法术攻击力

	 COMPUTE( p27defence )			    // 心意提升物防
	 COMPUTE( m27defence )			    // 心意提升魔防
	 COMPUTE( m27axhp )			        // 心意提升最大生命值
	 
	 // 绝技攻防计算 //by=>friday
	 COMPUTE( jj27_attack )
	 COMPUTE( jj27_defence )
...
/**     
 * \brief 心意提升降级 //by=>friday
 *
 * \param ob: 装备物品
 * \param base: 心意提升属性
 * \return 当前总是返回true
 */
  bool Upgrade::do_xyts_downgrade(zObject* ob, zUxytsObjectB* base)
{
	 if (ob->data.retain101 > 0) 
	 {
		 //by=>friday, 先用当前等级的配置减少属性，再减少心心等级
		 COMPUTE( p27damage )				// 心意提升最小攻击力
		 COMPUTE( m27axpdamage )			// 心意提升最大攻击力
		 COMPUTE( m27damage )			    // 心意提升最小法术攻击力
		 COMPUTE( m27axmdamage )			// 心意提升最大法术攻击力
 ...
 #define COMPUTE(x) ob->data.x += base->x;
 /**     
  * \brief 情谊提升升级 //by=>friday
  *
  * \param ob: 装备物品
  * \param base: 情谊提升属性
  * \return 当前总是返回true
  */
 bool Upgrade::do_qyts_upgrade(zObject* ob, zUqytsObjectB* base)
 {
	 //增加情谊提升标记 //by=>friday
	 ++ob->data.retain102;
	 
	 COMPUTE( p28damage )				// 情谊提升最小攻击力
	 COMPUTE( m28axpdamage )			// 情谊提升最大攻击力
	 COMPUTE( m28damage )			    // 情谊提升最小法术攻击力
	 COMPUTE( m28axmdamage )			// 情谊提升最大法术攻击力

	 COMPUTE( p28defence )			    // 情谊提升物防
	 COMPUTE( m28defence )			    // 情谊提升魔防
	 COMPUTE( m28axhp )			        // 情谊提升最大生命值
	 
	 // 绝技攻防计算 //by=>friday
	 COMPUTE( jj28_attack )
	 COMPUTE( jj28_defence )
...
/**     
 * \brief 情谊提升降级 //by=>friday
 *
 * \param ob: 装备物品
 * \param base: 情谊提升属性
 * \return 当前总是返回true
 */
  bool Upgrade::do_qyts_downgrade(zObject* ob, zUqytsObjectB* base)
{
	 if (ob->data.retain102 > 0) 
	 {
		 //by=>friday, 先用当前等级的配置减少属性，再减少情谊等级
		 COMPUTE( p28damage )				// 情谊提升最小攻击力
		 COMPUTE( m28axpdamage )			// 情谊提升最大攻击力
		 COMPUTE( m28damage )			    // 情谊提升最小法术攻击力
		 COMPUTE( m28axmdamage )			// 情谊提升最大法术攻击力
 ...
 #define COMPUTE(x) ob->data.x += base->x;
 /**     
  * \brief 定情信物进阶升级 //by=>friday
  *
  * \param ob: 装备物品
  * \param base: 定情信物进阶属性
  * \return 当前总是返回true
  */
 bool Upgrade::do_dqxwjj_upgrade(zObject* ob, zUdqxwjjObjectB* base)
 {
	 //增加定情信物进阶标记 //by=>friday
	 ++ob->data.retain103;
	 
	 COMPUTE( p29damage )				// 定情信物进阶最小攻击力
	 COMPUTE( m29axpdamage )			// 定情信物进阶最大攻击力
	 COMPUTE( m29damage )			    // 定情信物进阶最小法术攻击力
	 COMPUTE( m29axmdamage )			// 定情信物进阶最大法术攻击力

	 COMPUTE( p29defence )			    // 定情信物进阶物防
	 COMPUTE( m29defence )			    // 定情信物进阶魔防
	 COMPUTE( m29axhp )			        // 定情信物进阶最大生命值
	 
	 // 绝技攻防计算 //by=>friday
	 COMPUTE( jj29_attack )
	 COMPUTE( jj29_defence )
...
/**     
 * \brief 定情信物进阶降级 //by=>friday
 *
 * \param ob: 装备物品
 * \param base: 定情信物进阶属性
 * \return 当前总是返回true
 */
  bool Upgrade::do_dqxwjj_downgrade(zObject* ob, zUdqxwjjObjectB* base)
{
	 if (ob->data.retain103 > 0) 
	 {
		 //by=>friday, 先用当前等级的配置减少属性，再减少进阶等级
		 COMPUTE( p29damage )				// 定情信物进阶最小攻击力
		 COMPUTE( m29axpdamage )			// 定情信物进阶最大攻击力
		 COMPUTE( m29damage )			    // 定情信物进阶最小法术攻击力
		 COMPUTE( m29axmdamage )			// 定情信物进阶最大法术攻击力
 ...
 //装备助手开始/////////////////////////////////////////////////////////		
	 ...
	 case Cmd::RECAST_ITEM_COMPOSE29:  //王者升级
		 ...
		 break;
	 case Cmd::RECAST_ITEM_COMPOSE30:  //装备升心 //by=>friday
		 {
			 return equipCompose30(user,base, rev);
		 }
		 break;
	 case Cmd::RECAST_ITEM_COMPOSE31:  //心意提升 //by=>friday
		 {
			 return equipCompose31(user,base, rev);
		 }
		 break;
	 case Cmd::RECAST_ITEM_COMPOSE32:  //情谊提升 //by=>friday
		 {
			 return equipCompose32(user,base, rev);
		 }
		 break;
	 case Cmd::RECAST_ITEM_COMPOSE33:  //定情信物突破/进阶 //by=>friday
		 {
			 return equipCompose33(user,base, rev);
		 }
		 break;
 //附件助手开始/////////////////////////////////////////////////////////
 ...
 //图鉴助手开始/////////////////////////////////////////////////////////
	 ...
	 case Cmd::RECAST_ITEM_COMPOSE102:  //百兽图鉴签名
		 ...
		 break;			
	 case Cmd::RECAST_ITEM_COMPOSE109:  //by=>friday 孩子装备升级
		 {
			 return equipCompose109(user, base, rev);
		 }
		 break;
 //法宝助手开始/////////////////////////////////////////////////////////
 ...
 bool RebuildObject::equipCompose17(SceneUser &user, zObjectB *base, const Cmd::ItemRecastPropertyUserCmd *rev)
 {
	 ...
	 struct do_compose : public PackageCallback
	 {
		 ...
		 {
			 //by=>friday
			 if(up_ob->data.needlevel >= 215)
			 {
				if (up_ob->data.kind & 16 && up_ob->data.upgrade >= 34 )
				{
				   return count >= 1 && up_ob && is_stuff() && can;
				}
				else
	 ...
	 else
	 {
		 if (!dc.can_compose())
		 {
			if(dc.up_ob->data.needlevel >= 215)
			{
			 Base::reply(user, 1, EQUIP_COMPOSE17, "          需要34星才可以激活龙槽");
			}
			else
 ...
 bool RebuildObject::equipCompose18(SceneUser &user, zObjectB *base, const Cmd::ItemRecastPropertyUserCmd *rev)
 {
	 ...
	 struct do_compose : public PackageCallback
	 {
		 ...
		 bool can_compose()
		 {
			...
			if(up_ob->data.needlevel >= 215)
			{
 				if (up_ob->data.kind & 16 && up_ob->data.upgrade >= 34 && up_ob->data.dtrough >= 1)
				{
					return count >= 1 && up_ob && is_stuff() && can;
				}
				else
 ...
 /**
  * @brief 定情信物镶嵌服务器端处理函数 //by=>friday
  * @param user 请求玩家对象引用
  * @param base 物品基础数据指针（当前未使用）
  * @param rev 客户端发送的装备改造命令指针
 ...
 * \return 镶嵌是否成功
 */
bool RebuildObject::equipCompose30( SceneUser &user,zObjectB *base, const Cmd::ItemRecastPropertyUserCmd *rev ) //by=>friday
{
	 // 获取用户重铸包裹 //by=>friday
	 RecastPack* pack = &(user.packs.recastPack);
	 if (!pack) // 检查包裹是否存在 //by=>friday
	 {
		 Zebra::logger->alarm("用户(%d:%d:%s)请求定情信物镶嵌的包裹不存在", user.charbase.accid, user.charbase.id, user.charbase.name);
		 return false; // 包裹不存在返回失败 //by=>friday
	 }

	 struct do_compose : public PackageCallback
	 {
		 do_compose() : up_ob(NULL), c_ob(NULL), odds_ob(NULL), count(0), can(true), nextLevelObj(NULL)
		 { }
		 ...
		 int count;
		 bool can;
		 zUdqxwxqObjectB* nextLevelObj;

		 bool getNextLevelConfig()
		 {
			 if (!nextLevelObj && up_ob)
			 {
				//  DWORD nextLevelId = (up_ob->data.retain100 + 1) * 100000 + up_ob->data.dwObjectID;
				 DWORD nextLevelId = up_ob->data.dwObjectID;//by=>frifay 由于这里我只设计可以镶嵌一次
				 nextLevelObj = udqxwxqobjectbm.get(nextLevelId);
			 }
		 ...
		 bool is_stuff()
		 {
			 if (!getNextLevelConfig()) return false;
			 return c_ob->data.dwObjectID == nextLevelObj->stuff;
		 }
	 ...
	 // 使用缓存的配置，无需重复查询
	 if (!dc.getNextLevelConfig())
	 {
		 if (dc.up_ob->data.retain100 == 0)
		 {
			printf("物品id=%d\n",dc.up_ob->data.dwObjectID);
			 // 等级为0且没有下一级配置，说明此物品不支持定情信物镶嵌
			 Base::reply(user, 1, EQUIP_COMPOSE30, "此物品不可以镶嵌定情信物");
		 }
		 else
		 {
			 // 等级>0且没有下一级配置，说明已达到满级
			 Base::reply(user, 1, EQUIP_COMPOSE30, "定情信物镶嵌等级已经满级");
		 }
		 return false;			
	 }
	 if (dc.up_ob->data.retain100 >= 1)
	 {
		//  Base::reply(user, 1, EQUIP_COMPOSE30, "定情信物镶嵌等级已经满级");
		 Base::reply(user, 1, EQUIP_COMPOSE30, "定情信物已镶嵌");
		 return false;	
	 }
	 
	 if (dc.c_ob->data.dwObjectID != dc.nextLevelObj->stuff)
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE30, "材料不匹配");
		 return false;
	 }
	 else
	 {
		 if (!dc.can_compose()) 
		 {
			 Zebra::logger->alarm("用户(%d:%d:%s)请求传送非法定情信物镶嵌物品", user.charbase.accid, user.charbase.id, user.charbase.name);
			 Base::reply(user, 1, EQUIP_COMPOSE30, "错误物品不能定情信物镶嵌");
			 return false;			
		 }
	 }

	 DWORD need = dc.nextLevelObj->gold;
	 if (!user.packs.checkGold(need))
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE30, "金子不足");
		 return false;
	 }

	 if (user.packs.removeGold(need, "定情信物镶嵌"))
	 {

		//  DWORD originalID = dc.up_ob->data.dwObjectID;
		// //  DWORD nextLevelId = (dc.up_ob->data.retain100 + 1) * 100000 + dc.up_ob->data.dwObjectID;
		//  DWORD nextLevelId = dc.up_ob->data.dwObjectID;
		//  dc.up_ob->data.dwObjectID = nextLevelId;
		 
		 bool ret = Upgrade::dqxw_upgrade(user, dc.up_ob, dc.odds());
		 
		 // 恢复原始ID
		//  dc.up_ob->data.dwObjectID = originalID;

		 zObject* ob = zObject::create(dc.up_ob);
		 struct MultiItemDelCB : public PackageCallback
		 {
			 MultiItemDelCB(SceneUser* pUser, zObject* equipObj, DWORD materialId) : up_ob(equipObj), delObj(NULL), realDelCnt(0), pOwner(pUser), materialID(materialId)
			 { }

			 bool exec(zObject* ob)
			 {
				 if (ob->data.dwObjectID == materialID) 
				 {
					 // 处理材料删除
					 delObj = ob;
					 static const DWORD MAX_DEL_COUNT = 1;
					 if (pOwner && realDelCnt < MAX_DEL_COUNT)
					 {
						 ...
						 {
							 realDelNum = delObj->data.dwNum;
							 zObject::logger(delObj->createid,delObj->data.qwThisID,delObj->data.strName,delObj->data.dwNum,delObj->data.dwNum,0,pOwner->id,pOwner->name,0,NULL,"定情信物镶嵌删除",delObj->base,delObj->data.kind,delObj->data.retain100);
							 pOwner->packs.removeObject(delObj);
						 }
						 realDelCnt += realDelNum;
					 }
		 ...
		 MultiItemDelCB del(&user, dc.up_ob, dc.nextLevelObj->stuff);
		 pack->execEvery(del);
		 ...
		 {	
			 ...
			 //by=>friday, 定情信物镶嵌成功时，初始化历史最高心
			 if (ret)
			 {
				 ob->data.retain104 = 0; //by=>friday, 刚镶嵌时历史最高心为0
			 }

			 if (user.packs.addObject(ob, false)) 
			 {			
				 zObject::logger(ob->createid,ob->data.qwThisID,ob->data.strName,ob->data.dwNum,ob->data.dwNum,1,0,NULL,user.id,user.name,"定情信物镶嵌生成",ob->base,ob->data.kind,ob->data.retain100);
				 Base::add_object(user, ob, false);
				 Base::reply(user, ret?0:1, EQUIP_COMPOSE30, ret?"             定情信物镶嵌成功":"             定情信物镶嵌失败");
			 }
		 }
		 return true;
	 }
 ...
 */
//心意提升 //by=>friday
bool RebuildObject::equipCompose31( SceneUser &user,zObjectB *base, const Cmd::ItemRecastPropertyUserCmd *rev ) //by=>friday
{
	 // 获取用户重铸包裹 //by=>friday
	 RecastPack* pack = &(user.packs.recastPack);
	 if (!pack) // 检查包裹是否存在 //by=>friday
	 ...
	 struct do_compose : public PackageCallback
	 {
		 ...
		 int count;
		 bool can;
		 zUxytsObjectB* nextLevelObj;  // 缓存下一级配置

		 bool getNextLevelConfig()
		 {
			 if (!nextLevelObj && up_ob)
			 {
				 DWORD nextLevelId = (up_ob->data.retain101 + 1) * 100000 + up_ob->data.dwObjectID;
				 nextLevelObj = uxytsobjectbm.get(nextLevelId);
			 }
			 return nextLevelObj != NULL;
		 }
	 ...
	 if (!dc.up_ob || !dc.c_ob ) 
	 {
		 Zebra::logger->alarm("用户(%d:%d:%s)传送时内容非法", user.charbase.accid, user.charbase.id, user.charbase.name);
		 return false;			
	 }
	 ...
	 if (dc.up_ob->data.retain100 < 1)
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE31, "请先镶嵌定情信物");
		 return false;
	 }
	 ...
	 if (!dc.getNextLevelConfig())
	 {
		 if (dc.up_ob->data.retain101 == 0)
		 {
			 // 等级为0且没有下一级配置，说明此物品不支持心意提升
			 Base::reply(user, 1, EQUIP_COMPOSE31, "此物品不可以提升心意");
		 }
		 else
		 {
			 // 等级>0且没有下一级配置，说明已达到满级
			 Base::reply(user, 1, EQUIP_COMPOSE31, "心意提升等级已经满级");
		 }
		 return false;			
	 }
	 ...
	 if (dc.c_ob->data.dwObjectID != dc.nextLevelObj->stuff)
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE31, "材料不匹配");
		 return false;
	 }
	 else
	 {
		 if (!dc.can_compose()) 
		 {
			 Zebra::logger->alarm("用户(%d:%d:%s)请求传送非法心意提升物品", user.charbase.accid, user.charbase.id, user.charbase.name);
			 Base::reply(user, 1, EQUIP_COMPOSE31, "错误物品不能心意提升");
			 return false;			
		 }
	 }
	 ...
	 if (!user.packs.checkGold(need))
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE31, "金子不足");
		 return false;
	 }

	 if (user.packs.removeGold(need, "心意提升"))
	 {
		//  // 临时修改物品ID为下一级ID，让upgrade函数获取正确的配置
		//  DWORD originalID = dc.up_ob->data.dwObjectID;
		//  DWORD nextLevelId = (dc.up_ob->data.retain101 + 1) * 100000 + dc.up_ob->data.dwObjectID;
		//  dc.up_ob->data.dwObjectID = nextLevelId;
		 
		 bool ret = Upgrade::xyts_upgrade(user, dc.up_ob, dc.odds());
		...
		//  dc.up_ob->data.dwObjectID = originalID;
			 ...
			 bool exec(zObject* ob)
			 {
				 if (ob->data.dwObjectID == materialID) 
				 {
					 ...
					 if (pOwner && realDelCnt < MAX_DEL_COUNT)
					 {
						 ...
						 {
							 realDelNum = delObj->data.dwNum;
							 zObject::logger(delObj->createid,delObj->data.qwThisID,delObj->data.strName,delObj->data.dwNum,delObj->data.dwNum,0,pOwner->id,pOwner->name,0,NULL,"心意提升删除",delObj->base,delObj->data.kind,delObj->data.retain100);
							 pOwner->packs.removeObject(delObj);
						 }
						 realDelCnt += realDelNum;
					 }
		 ...
		 {	
			 ...
			 //by=>friday, 成功和失败逻辑已在xyts_upgrade中处理

			 if (user.packs.addObject(ob, false)) 
			 {			
				 zObject::logger(ob->createid,ob->data.qwThisID,ob->data.strName,ob->data.dwNum,ob->data.dwNum,1,0,NULL,user.id,user.name,"心意提升生成",ob->base,ob->data.kind,ob->data.retain100);
				 Base::add_object(user, ob, false);
				 Base::reply(user, ret?0:1, EQUIP_COMPOSE31, ret?"             心意提升成功":"             心意提升失败");
			 }
		 }
		 return true;
	 }
 ...
 */
//情谊提升 //by=>friday
bool RebuildObject::equipCompose32( SceneUser &user,zObjectB *base, const Cmd::ItemRecastPropertyUserCmd *rev ) //by=>friday
{
	 // 获取用户重铸包裹 //by=>friday
	 RecastPack* pack = &(user.packs.recastPack);
	 if (!pack) // 检查包裹是否存在 //by=>friday
	 {
		 Zebra::logger->alarm("用户(%d:%d:%s)请求情谊提升的包裹不存在", user.charbase.accid, user.charbase.id, user.charbase.name);
		 return false; // 包裹不存在返回失败 //by=>friday
	 }
	 ...
	 struct do_compose : public PackageCallback
	 {
		 ...
		 int count;
		 bool can;
		 zUqytsObjectB* nextLevelObj;

		 bool getNextLevelConfig()
		 {
			 if (!nextLevelObj && up_ob)
			 {
				 DWORD nextLevelId = (up_ob->data.retain102 + 1) * 100000 + up_ob->data.dwObjectID;
				 nextLevelObj = uqytsobjectbm.get(nextLevelId);
			 }
			 return nextLevelObj != NULL;
		 }
	 ...
	 if (dc.up_ob->data.retain100 < 1)
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE32, "请先镶嵌定情信物");
		 return false;
	 }
	 ...
	 if (!dc.getNextLevelConfig())
	 {
		 if (dc.up_ob->data.retain102 == 0)
		 {
			 // 等级为0且没有下一级配置，说明此物品不支持情谊提升
			 Base::reply(user, 1, EQUIP_COMPOSE32, "此物品不可以提升情谊");
		 }
		 else
		 {
			 // 等级>0且没有下一级配置，说明已达到满级
			 Base::reply(user, 1, EQUIP_COMPOSE32, "情谊提升等级已经满级");
		 }
		 return false;			
	 }
	 ...
	 if (dc.c_ob->data.dwObjectID != dc.nextLevelObj->stuff)
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE32, "材料不匹配");
		 return false;
	 }
	 else
	 {
		 if (!dc.can_compose()) 
		 {
			 Zebra::logger->alarm("用户(%d:%d:%s)请求传送非法情谊提升物品", user.charbase.accid, user.charbase.id, user.charbase.name);
			 Base::reply(user, 1, EQUIP_COMPOSE32, "错误物品不能情谊提升");
			 return false;			
		 }
	 }
	 ...
	 if (!user.packs.checkGold(need))
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE32, "金子不足");
		 return false;
	 }

	 if (user.packs.removeGold(need, "情谊提升"))
	 {
		//  // 临时修改物品ID为下一级ID，让upgrade函数获取正确的配置
		//  DWORD originalID = dc.up_ob->data.dwObjectID;
		//  DWORD nextLevelId = (dc.up_ob->data.retain102 + 1) * 100000 + dc.up_ob->data.dwObjectID;
		//  dc.up_ob->data.dwObjectID = nextLevelId;
		 
		 bool ret = Upgrade::qyts_upgrade(user, dc.up_ob, dc.odds());
		...
		//  dc.up_ob->data.dwObjectID = originalID;
			 ...
			 bool exec(zObject* ob)
			 {
				 if (ob->data.dwObjectID == materialID) 
				 {
					 ...
					 if (pOwner && realDelCnt < MAX_DEL_COUNT)
					 {
						 ...
						 {
							 realDelNum = delObj->data.dwNum;
							 zObject::logger(delObj->createid,delObj->data.qwThisID,delObj->data.strName,delObj->data.dwNum,delObj->data.dwNum,0,pOwner->id,pOwner->name,0,NULL,"情谊提升删除",delObj->base,delObj->data.kind,delObj->data.retain100);
							 pOwner->packs.removeObject(delObj);
						 }
						 realDelCnt += realDelNum;
					 }
		 ...
		 {	
			 ...
			 if (user.packs.addObject(ob, false)) 
			 {			
				 zObject::logger(ob->createid,ob->data.qwThisID,ob->data.strName,ob->data.dwNum,ob->data.dwNum,1,0,NULL,user.id,user.name,"情谊提升生成",ob->base,ob->data.kind,ob->data.retain100);
				 Base::add_object(user, ob, false);
				 Base::reply(user, ret?0:1, EQUIP_COMPOSE32, ret?"             情谊提升成功":"             情谊提升失败");
			 }
		 }
		 return true;
	 }
 ...
 * \param base: 基础物品对象  
 * \param rev: 物品重铸属性命令
 * \return 定情信物突破/进阶是否成功
 */
//定情信物突破/进阶 //by=>friday
bool RebuildObject::equipCompose33( SceneUser &user,zObjectB *base, const Cmd::ItemRecastPropertyUserCmd *rev ) //by=>friday
{
	 // 获取用户重铸包裹 //by=>friday
	 RecastPack* pack = &(user.packs.recastPack);
	 if (!pack) // 检查包裹是否存在 //by=>friday
	 {
		 Zebra::logger->alarm("用户(%d:%d:%s)请求定情信物突破/进阶的包裹不存在", user.charbase.accid, user.charbase.id, user.charbase.name);
		 return false; // 包裹不存在返回失败 //by=>friday
	 }
	 ...
	 struct do_compose : public PackageCallback
	 {
		 ...
		 int count;
		 bool can;
		 zUdqxwjjObjectB* nextLevelObj;

		 bool getNextLevelConfig()
		 {
			 if (!nextLevelObj && up_ob)
			 {
				 DWORD nextLevelId = (up_ob->data.retain103 + 1) * 100000 + up_ob->data.dwObjectID;
				 nextLevelObj = udqxwjjobjectbm.get(nextLevelId);
			 }
			 return nextLevelObj != NULL;
		 }
	 ...
	 if (dc.up_ob->data.retain100 < 1)
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE33, "请先镶嵌定情信物");
		 return false;
	 }
	 ...
	 if (!dc.getNextLevelConfig())
	 {
		 if (dc.up_ob->data.retain103 == 0)
		 {
			 // 等级为0且没有下一级配置，说明此物品不支持定情信物突破/进阶
			 Base::reply(user, 1, EQUIP_COMPOSE33, "此物品不可以突破/进阶");
		 }
		 else
		 {
			 // 等级>0且没有下一级配置，说明已达到满级
			 Base::reply(user, 1, EQUIP_COMPOSE33, "定情信物突破/进阶等级已经满级");
		 }
		 return false;			
	 }
	 ...
	 if (dc.c_ob->data.dwObjectID != dc.nextLevelObj->stuff)
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE33, "材料不匹配");
		 return false;
	 }
	 else
	 {
		 if (!dc.can_compose()) 
		 {
			 Zebra::logger->alarm("用户(%d:%d:%s)请求传送非法定情信物突破/进阶物品", user.charbase.accid, user.charbase.id, user.charbase.name);
			 Base::reply(user, 1, EQUIP_COMPOSE33, "错误物品不能定情信物突破/进阶");
			 return false;			
		 }
	 }
	 ...
	 if (!user.packs.checkGold(need))
	 {
		 Base::reply(user, 1, EQUIP_COMPOSE33, "金子不足");
		 return false;
	 }

	 if (user.packs.removeGold(need, "定情信物突破/进阶"))
	 {
		//  // 临时修改物品ID为下一级ID，让upgrade函数获取正确的配置
		//  DWORD originalID = dc.up_ob->data.dwObjectID;
		//  DWORD nextLevelId = (dc.up_ob->data.retain103 + 1) * 100000 + dc.up_ob->data.dwObjectID;
		//  dc.up_ob->data.dwObjectID = nextLevelId;
		 
		 bool ret = Upgrade::dqxwjj_upgrade(user, dc.up_ob, dc.odds());
		...
		//  dc.up_ob->data.dwObjectID = originalID;
			 ...
			 bool exec(zObject* ob)
			 {
				 if (ob->data.dwObjectID == materialID) 
				 {
					 ...
					 if (pOwner && realDelCnt < MAX_DEL_COUNT)
					 {
						 ...
						 {
							 realDelNum = delObj->data.dwNum;
							 zObject::logger(delObj->createid,delObj->data.qwThisID,delObj->data.strName,delObj->data.dwNum,delObj->data.dwNum,0,pOwner->id,pOwner->name,0,NULL,"定情信物突破/进阶删除",delObj->base,delObj->data.kind,delObj->data.retain100);
							 pOwner->packs.removeObject(delObj);
						 }
						 realDelCnt += realDelNum;
					 }
		 ...
		 {	
			 ...
			 if (user.packs.addObject(ob, false)) 
			 {			
				 zObject::logger(ob->createid,ob->data.qwThisID,ob->data.strName,ob->data.dwNum,ob->data.dwNum,1,0,NULL,user.id,user.name,"定情信物突破/进阶生成",ob->base,ob->data.kind,ob->data.retain100);
				 Base::add_object(user, ob, false);
				 Base::reply(user, ret?0:1, EQUIP_COMPOSE33, ret?"             定情信物突破/进阶成功":"             定情信物突破/进阶失败");
			 }
		 }
		 return true;
	 }
 ...
 /**
  * @brief 孩子装备升级，材料数量正确扣除，装备属性安全升级，自动同步客户端 //by=>friday
  */
 bool RebuildObject::equipCompose109(SceneUser &user, zObjectB *base, const Cmd::ItemRecastPropertyUserCmd *rev)
 {
	 // 获取附件包裹//by=>friday
	 BabyRecastPack* pack = &(user.packs.babyRecastPack);
	 if (!pack) 
	 { 
		 Zebra::logger->alarm("用户(%d:%d:%s)请求孩子装备升级的包裹不存在", user.charbase.accid, user.charbase.id, user.charbase.name);
		 return false;
	 } 
	 
	 // 从孩子装备升级包裹中获取孩子装备和材料 //by=>friday
	 zObject* babyEquip = NULL;
	 zObject* material = NULL;
	 
	 struct GetBabyEquipItems : public PackageCallback 
	 { 
	 public: 
		 GetBabyEquipItems() : babyEquip(NULL), material(NULL) {} 
		 
		 bool exec(zObject* ob) 
		 { 
			 // 检查是否为孩子装备 //by=>friday
			 if (ob->base->kind >= ItemType_BABYFJ1 && ob->base->kind <= ItemType_BABYFJ6)
			 { 
				 if (!babyEquip) 
				 { 
					 babyEquip = ob;
				 ...
				 { 
					 Zebra::logger->alarm("附件包裹中有多个孩子装备");
					 return false;
				 } 
			 } 
			 else 
	 ...
	 GetBabyEquipItems getter;
	 pack->execEvery(getter);
	 
	 // 检查是否找到了孩子装备和材料 //by=>friday
	 if (!getter.babyEquip || !getter.material) 
	 { 
		 // Zebra::logger->alarm("用户(%d:%d:%s)附件包裹中缺少孩子装备或材料", user.charbase.accid, user.charbase.id, user.charbase.name);
		 Base::reply(user, 1, EQUIP_COMPOSE109, "请放入孩子装备和升级材料"); //by=>friday
		 return false;
	 } 
	 
	 // 获取装备当前等级，计算下一级的升级表ID //by=>friday
	 DWORD currentLevel = getter.babyEquip->data.retain35;
	 DWORD nextLevel = currentLevel + 1;
 
	 // 计算升级表ID：等级*100000 + 装备ID //by=>friday
	 DWORD upgradeTableID = nextLevel * 100000 + getter.babyEquip->base->id;
	 
	 // 查找升级表 //by=>friday
	 zBabyEquipUpgradeB* upgradeInfo = babyequipupgradebm.get(upgradeTableID);
	 if (!upgradeInfo) 
	 {
		 Zebra::logger->alarm("用户(%d:%d:%s)找不到装备ID=%d的%d级升级配置，升级表ID=%d", 
			 user.charbase.accid, user.charbase.id, user.charbase.name, 
			 getter.babyEquip->base->id, nextLevel, upgradeTableID);
		 Base::reply(user, 1, EQUIP_COMPOSE109, "装备等级已达上限"); //by=>friday
		 return false;
	 } 
	 
	 // 检查材料是否匹配 //by=>friday
	 if (getter.material->base->id != upgradeInfo->stuff) 
	 { 
		 // Zebra::logger->alarm("用户(%d:%d:%s)升级材料不匹配，需要材料ID=%d，实际材料ID=%d", 
		 //     user.charbase.accid, user.charbase.id, user.charbase.name, 
		 //     upgradeInfo->stuff, getter.material->base->id);
		 Base::reply(user, 1, EQUIP_COMPOSE109, "升级材料不正确"); //by=>friday
		 return false;
	 } 
	 
	 // 检查材料数量是否足够 //by=>friday
	 if (getter.material->data.dwNum < upgradeInfo->materialCount)
	 { 
		 // Zebra::logger->alarm("用户(%d:%d:%s)材料数量不足，需要%d个，实际%d个", 
		 //     user.charbase.accid, user.charbase.id, user.charbase.name, 
		 //     upgradeInfo->materialCount, getter.material->data.dwNum);
		 Base::reply(user, 1, EQUIP_COMPOSE109, "升级材料数量不足"); //by=>friday
		 return false;
	 } 
	 
	 // 检查金钱 //by=>friday
	 if (!user.packs.checkGold(upgradeInfo->gold))
	 { 
		 
		 // Zebra::logger->alarm("用户(%d:%d:%s)金钱不足，需要=%d", 
		 //     user.charbase.accid, user.charbase.id, user.charbase.name, upgradeInfo->gold);
		Base::reply(user, 1, EQUIP_COMPOSE109, "金钱不足，无法升级"); //by=>friday
		 return false;
	 } 
	 
	 // 消耗金钱 //by=>friday
	 user.packs.removeGold(upgradeInfo->gold, "孩子装备升级");
	 
	 // 扣除材料数量 //by=>friday
	 getter.material->data.dwNum -= upgradeInfo->materialCount;
	 
	 // 发送材料数量更新消息给客户端 //by=>friday
	 if (getter.material->data.dwNum > 0)
	 { 
		 // 材料还有剩余，发送数量更新消息 //by=>friday
		 Cmd::stRefCountObjectPropertyUserCmd send;
		 send.qwThisID = getter.material->data.qwThisID;
		 send.dwNum = getter.material->data.dwNum;
		 user.sendCmdToMe(&send, sizeof(send));
	 } 
	 else 
	 { 
		 // 材料用完，删除物品 //by=>friday
		 user.packs.removeObject(getter.material, true, true);
	 } 
	 //////////////////////////////////////////////////////////////
	 // 升级装备属性 //by=>friday
	 if (upgradeInfo->pdamage > 0)
	 {
		 getter.babyEquip->data.pdamage += upgradeInfo->pdamage;
	 }
	 if (upgradeInfo->maxpdamage > 0)
	 {
		 getter.babyEquip->data.maxpdamage += upgradeInfo->maxpdamage;
	 }
	 if (upgradeInfo->mdamage > 0)
	 {
		 getter.babyEquip->data.mdamage += upgradeInfo->mdamage;
	 }
	 if (upgradeInfo->maxmdamage > 0)
	 {
		 getter.babyEquip->data.maxmdamage += upgradeInfo->maxmdamage;
	 }
	 if (upgradeInfo->pdefence > 0)
	 {
		 getter.babyEquip->data.pdefence += upgradeInfo->pdefence;
	 }
	 if (upgradeInfo->mdefence > 0)
	 {
		 getter.babyEquip->data.mdefence += upgradeInfo->mdefence;
	 }
	 if (upgradeInfo->maxhp > 0)
	 {
		 getter.babyEquip->data.maxhp += upgradeInfo->maxhp;
	 }
	 ...
	 // 在服务端升级成功后添加调试日志
 // Zebra::logger->debug("用户(%d:%d:%s)孩子装备升级成功，装备ID=%d，装备唯一ID=%llu，从%d级升级到%d级", 
 //     user.charbase.accid, user.charbase.id, user.charbase.name, 
 //     getter.babyEquip->base->id, getter.babyEquip->data.qwThisID, currentLevel, getter.babyEquip->data.retain35);
 
 
	 // 发送装备更新消息给客户端 //by=>friday
	 Base::add_object(user, getter.babyEquip, false);
	 
	 // 发送升级成功消息给客户端 //by=>friday
	 Channel::sendSys(&user, Cmd::INFO_TYPE_GAME, "孩子装备升级成功！装备等级提升到%d级", nextLevel);
	 
	 // Zebra::logger->debug("用户(%d:%d:%s)孩子装备升级成功，装备ID=%d，从%d级升级到%d级", 
	 //     user.charbase.accid, user.charbase.id, user.charbase.name, 
	 //     getter.babyEquip->base->id, currentLevel, nextLevel);
	 Base::reply(user, 0, EQUIP_COMPOSE109, "孩子装备升级成功！"); //by=>friday
	 return true;
 }
