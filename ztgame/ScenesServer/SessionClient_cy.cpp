...
bool SessionClient::cmdMsgParse_Other(const Cmd::t_NullCmd *ptNullCmd, const unsigned int nCmdLen)
{
	...
	if (ptNullCmd && ptNullCmd->cmd == Cmd::Session::CMD_SCENE && ptNullCmd->para == Cmd::Session::PARA_SCENE_REGUSER)
	{
		Zebra::logger->debug("DBG_SCENE_OTHER_ENTER cmd=%u para=%u len=%u",
				ptNullCmd->cmd,
				ptNullCmd->para,
				nCmdLen);
	}
		...
		case Cmd::Session::PARA_SCENE_REGUSER:
			{
				...
				Zebra::logger->debug("DBG_SCENE_REGUSER_RECV accid=%u id=%u tempid=%u mapid=%u map=%s gatewayServerID=%u cmdLen=%u",
						rev->accid,
						rev->dwID,
						rev->dwTempID,
						rev->dwMapID,
						(char *)rev->byMapName,
						rev->dwGatewayServerID,
						nCmdLen);
				Scene *scene=SceneManager::getInstance() .getSceneByName((char *)rev->byMapName);
				Zebra::logger->debug("DBG_SCENE_REGUSER_SCENE first scene=%p map=%s", scene, (char *)rev->byMapName);
				SceneTask *gate=SceneTaskManager::getInstance().uniqueGet(rev->dwGatewayServerID);
				Zebra::logger->debug("DBG_SCENE_REGUSER_GATE gate=%p gatewayServerID=%u", gate, rev->dwGatewayServerID);
				if(!scene)
				{       
					...
					bcopy("清源村",&map[6],6);
					Zebra::logger->debug("DBG_SCENE_REGUSER_SCENE fallback_try map=%s", map);
					scene=SceneManager::getInstance().getSceneByName(map);
					Zebra::logger->debug("DBG_SCENE_REGUSER_SCENE fallback scene=%p map=%s", scene, map);
				}   
				if(scene)
				{
					Zebra::logger->debug("DBG_SCENE_REGUSER_SCENE_OK scene=%p sceneTempID=%u realMapID=%u", scene, scene->tempid, scene->getRealMapID());
					if(gate)
					{
						Zebra::logger->debug("DBG_SCENE_REGUSER_GATE_OK gate=%p gatewayServerID=%u", gate, rev->dwGatewayServerID);
						// 添加用户
						//						SceneUser *pUser=new SceneUserVisitor(rev->accid);
						SceneRecycleUserManager::getInstance().refresh();
						Zebra::logger->debug("DBG_SCENE_REGUSER_CANREG_CHECK id=%u", rev->dwID);
						if(SceneRecycleUserManager::getInstance().canReg(rev->dwID))
						{
							Zebra::logger->debug("DBG_SCENE_REGUSER_CANREG_OK id=%u", rev->dwID);
							...
							{
								Zebra::logger->debug("DBG_SCENE_REGUSER_ADDUSER_OK accid=%u id=%u tempid=%u name=%s sceneTempID=%u mapid=%u",
										pUser->accid,
										pUser->id,
										pUser->tempid,
										pUser->name,
								...
								Zebra::logger->debug("DBG_SCENE_REGUSER_BEFORE_RECORD accid=%u id=%u mapTempID=%u", send.accid, send.id, send.dwMapTempID);
								bool sendRecordOK = recordClient->sendCmd(&send,sizeof(send));
								Zebra::logger->debug("DBG_SCENE_REGUSER_SEND_RECORD ok=%d accid=%u id=%u mapTempID=%u", sendRecordOK ? 1 : 0, send.accid, send.id, send.dwMapTempID);
...
bool SessionClient::cmdMsgParse(const Cmd::t_NullCmd *ptNullCmd, const unsigned int nCmdLen)
{
	...
	if (ptNullCmd && ptNullCmd->cmd == Cmd::Session::CMD_SCENE && ptNullCmd->para == Cmd::Session::PARA_SCENE_REGUSER)
	{
		Zebra::logger->debug("DBG_SCENE_DISPATCH_ENTER cmd=%u para=%u len=%u",
				ptNullCmd->cmd,
				ptNullCmd->para,
				nCmdLen);
	}
	...
	switch(ptNullCmd->cmd)
	{
		...
		default:
			if (ptNullCmd && ptNullCmd->cmd == Cmd::Session::CMD_SCENE && ptNullCmd->para == Cmd::Session::PARA_SCENE_REGUSER)
			{
				Zebra::logger->debug("DBG_SCENE_DISPATCH_TO_OTHER cmd=%u para=%u len=%u",
						ptNullCmd->cmd,
						ptNullCmd->para,
						nCmdLen);
			}
