...
namespace WindowsFormsApplication1
{
    ...
    {
        // JSON 地址
        private const string JSON_URL = "http://e.haory.cn/e/e7/08/list2.json";
    ...
    _gameFilePath = GuessGameExe();
        ...
        private void AddServerFromList(string groupName, IList sa, string srvNameOverride)
        {
            ...
            else
            {
                ...
                name = srvNameOverride ?? (sa[0] as string) ?? "未知区服";
                ip = (sa.Count > 1 ? sa[1] as string : null) ?? "127.0.0.1";
                port = (sa.Count > 2 ? ToInt(sa[2]) : 7000);
                zone = (sa.Count > 3 ? ToInt(sa[3]) : 1);
            }
        ...
        // loginaddress / loginport / servername / zone 全部来自 JSON 的 ServerItem
        private void UpdateConfigIni(ServerItem s)
        {
            ...
            // 2）如果已有 config.ini，只改 [server] 段里的 4 个关键字段，其它保持原样
