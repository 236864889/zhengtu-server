...
## 数据库与账号体系线索
- `base/zDatabase*`、`zMysqlDBConnPool.cpp` 提供DB访问基础设施。
- `FLServer` 中 `LoginManager`、`DBAccessClient*` 指向登录与DB交互链路。
- `GatewayServer/LoginSessionManager` 与 `SessionServer` 共同参与会话链路（细节待确认）。
